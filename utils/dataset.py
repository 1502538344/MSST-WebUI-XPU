...
def prepare_data(
    ...
    if dist.is_initialized():
        ...
        if platform.system() == "Windows" and num_workers > 0:
            print(
                f"Warning: Windows detected. Setting num_workers=0 to avoid multiprocessing issues with XPU."
            )
            num_workers = 0
    ...
    def load_source(self, metadata, instr):
        ...
        while True:
            ...
            if self.dataset_type in [1, 4, 5, 6, 7]:
                ...
                if source is None:
                    file_missing = True
                    if should_print and not getattr(
                        self, f"_warned_missing_{instr}", False
                    ):
                        print(
                            f'Warning: No file found for instrument "{instr}" in {track_path} (this warning will only be shown once)'
                        )
                        setattr(self, f"_warned_missing_{instr}", True)
                    source = np.zeros((2, self.chunk_size), dtype=np.float32)
            else:
            ...
            # Break if file was missing (avoid infinite loop with zero array)
            # or if chunk is loud enough, or max retries reached
            if (
                file_missing
                or np.abs(source).mean() >= self.min_mean_abs
