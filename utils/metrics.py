...
def L1Freq_metric(
    ...
    reference = torch.from_numpy(reference).to("cpu")
    estimate = torch.from_numpy(estimate).to("cpu")
...
def LogWMSE_metric(
    ...
    reference = torch.from_numpy(reference).unsqueeze(0).unsqueeze(0).to("cpu")
    estimate = torch.from_numpy(estimate).unsqueeze(0).unsqueeze(0).to("cpu")
    mixture = torch.from_numpy(mixture).unsqueeze(0).to("cpu")
...
def AuraSTFT_metric(
    ...
    reference = torch.from_numpy(reference).unsqueeze(0).to("cpu")
    estimate = torch.from_numpy(estimate).unsqueeze(0).to("cpu")
...
def AuraMRSTFT_metric(
    ...
    reference = torch.from_numpy(reference).unsqueeze(0).float().to("cpu")
    estimate = torch.from_numpy(estimate).unsqueeze(0).float().to("cpu")
...
def bleed_full(
    ...
    reference = torch.from_numpy(reference).float().to("cpu")
    estimate = torch.from_numpy(estimate).float().to("cpu")

    window = torch.hann_window(n_fft).to("cpu")
    ...
    mel_filter_bank = torch.from_numpy(mel_basis).to("cpu")
