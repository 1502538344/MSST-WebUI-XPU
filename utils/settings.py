...
def parse_args_train(
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override batch_size from config. If not set, uses config value.",
    )
    parser.add_argument(
...
def manual_seed(seed: int) -> None:
    ...
    try:
        ...
        if torch.xpu.is_available():
            ...
            torch.xpu.manual_seed_all(seed)  # if multi-GPU
...
def setup_ddp(rank: int, world_size: int, seed: int) -> None:
    ...
    initialize the process group with the CCL backend (for Intel XPU),
    and falls back to the Gloo backend if CCL is unavailable. Also sets the
    current XPU device to match the process rank.
    ...
    # Try CCL backend for Intel XPU, fallback to gloo
    try:
        ...
        if dist.get_rank() == 0:
            print('CCL not available. Using "gloo" backend.')
