...
from utils.device_utils import is_xpu_available
...
def train_one_epoch(
    ...
    device_type = "xpu" if is_xpu_available() else "cpu"
...
def compute_epoch_metrics(
    ...
    should_print = not dist.is_initialized() or dist.get_rank() == 0
    if not ddp:
        if is_xpu_available() and len(device_ids) > 1:
...
def train_model(
    ...
    if ddp:
        ...
        batch_size = config_batch_size * len(device_ids)
    ...
    if ddp:
        device = (
            torch.device(f"xpu:{rank}") if is_xpu_available() else torch.device(f"cpu")
        )
        model.to(device)
    ...
    if args.pre_valid:
        ...
        if ddp:
            ...
            if is_xpu_available() and len(args.device_ids) > 1:
                valid_multi_gpu(
    ...
    if args.set_per_process_memory_fraction and is_xpu_available():
        # XPU doesn't have set_per_process_memory_fraction, skip
        pass
    ...
    if should_print:
        if world_size:
            print_batch_size = config_batch_size
            ef_batch_size = print_batch_size * gradient_accumulation_steps * world_size
            num_gpu = world_size
        else:
            device_ids = args.device_ids
            print_batch_size = config_batch_size * len(device_ids)
            ef_batch_size = print_batch_size * gradient_accumulation_steps
