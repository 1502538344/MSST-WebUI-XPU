...
def process_audio_files(
    ...
    for path in mixture_paths:
        ...
        # Clear GPU memory after each song
...
def valid_multi_gpu(
    ...
    if ddp_mode:
        ...
        device = (
            torch.device(f"xpu:{rank}") if is_xpu_available() else torch.device("cpu")
        )
        model.to(device)
        model.eval()
...
def check_validation(dict_args):
    ...
    if is_xpu_available():
        device = torch.device(f"xpu:{device_ids[0]}")
    else:
        device = "cpu"
        print("XPU is not available. Run validation on CPU. It will be very slow...")
